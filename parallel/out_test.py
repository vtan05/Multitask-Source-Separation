...
def main(args):
    ...
    for track in tracks:
        ...
        # Predict
        audio, sr = librosa.load("D:/Data/eval/" + track + ".wav", offset=70, duration=3, mono=True, sr=16000)
        ...
        # source_spec = norbert.wiener(sources_reshape, orig_reshape, use_softmask=False)
        ...
        target_pred_mag_vocal = np.reshape(sources[:,:,:,0], (1025, 94))
        target_pred_mag_bas = np.reshape(sources[:,:,:,1], (1025, 94))
        target_pred_mag_dru = np.reshape(sources[:,:,:,2], (1025, 94))
        target_pred_mag_oth = np.reshape(sources[:,:,:,3], (1025, 94))
        target_pred_mag_inp = np.reshape(inp, (1025, 94))

        voc_spec = librosa.util.softmask(np.abs(target_pred_mag_vocal), np.abs(target_pred_mag_bas + target_pred_mag_dru + target_pred_mag_oth), power=2)
        bas_spec = librosa.util.softmask(np.abs(target_pred_mag_bas), np.abs(target_pred_mag_vocal + target_pred_mag_dru + target_pred_mag_oth), power=2)
        dru_spec = librosa.util.softmask(np.abs(target_pred_mag_dru), np.abs(target_pred_mag_vocal + target_pred_mag_bas + target_pred_mag_oth), power=2)
        oth_spec = librosa.util.softmask(np.abs(target_pred_mag_oth), np.abs(target_pred_mag_vocal + target_pred_mag_bas + target_pred_mag_dru), power=2)

        inp_istft = librosa.core.istft(target_pred_mag_inp, hop_length=hop_size, window=window, win_length=window_size)
        write_wav("defense/" + track + "/pred_input1_1.wav", inp_istft, sr, norm=False)

        if(sourceclass[0][0] <= 0.5):
            voc_istft = np.zeros((inp_istft.shape[0], 1))
        else:
            voc_istft = librosa.core.istft(voc_spec, hop_length=hop_size, window=window, win_length=window_size)
        write_wav("defense/" + track + "/pred_vocal1_1.wav", voc_istft, sr, norm=False)

        if(sourceclass[0][1] <= 0.5):
            bas_istft = np.zeros((inp_istft.shape[0], 1))
        else:
            bas_istft = librosa.core.istft(bas_spec, hop_length=hop_size, window=window, win_length=window_size)
        write_wav("defense/" + track + "/pred_bass1_1.wav", bas_istft, sr, norm=False)

        if(sourceclass[0][2] <= 0.5):
            dru_istft = np.zeros((inp_istft.shape[0], 1))
        else:
            dru_istft = librosa.core.istft(dru_spec, hop_length=hop_size, window=window, win_length=window_size)
        write_wav("defense/" + track + "/pred_drums1_1.wav", dru_istft, sr, norm=False)

        if(sourceclass[0][3] <= 0.5):
            others_istft = np.zeros((inp_istft.shape[0], 1))
        else:
            others_istft = librosa.core.istft(oth_spec, hop_length=hop_size, window=window, win_length=window_size)
        write_wav("defense/" + track + "/pred_others1_1.wav", others_istft, sr, norm=False)
